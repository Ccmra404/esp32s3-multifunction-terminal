...
- `assets` 在线资源包升级在传输中断后会自动保留下载地址，并允许覆盖修复损坏的
  assets 分区；擦除改为升级前批量执行，避免 6 MB 级主题包因逐扇区擦除耗时过长
  而中断。
- ST7789 使用杜邦线连接时把 SPI 时钟降到 40 MHz，消除 80 MHz 下的屏幕闪烁。
...
症状：第一次在线升级主题包时传输中断，之后网页一直提示升级失败。

原因有三处，均出现在上游当前代码中：

1. 固件在开始下载前就删除了 `download_url`，失败后没有重试入口
2. 启动时只要发现 `assets` 分区头无效就直接返回，不再读取待下载地址，
   因此第一次写坏分区后设备无法自修
3. 每收到 4 KB 就单独擦除一个扇区，6 MB 级资源包在本地验证时耗时超过十分钟，
   极容易在中途被网页超时或网络断开打断

**处理**：

- 下载成功后再删除 `download_url`
- 即使现有 `assets` 分区无效，只要存在待下载地址就允许覆盖修复
- 下载前一次性擦除目标区域，写入阶段不再逐扇区擦除
- HTTP body 达到声明的 `Content-Length` 即判定传输完成，不依赖连接优雅关闭

**验证**：使用网页生成的 `6,237,490` 字节主题包，从损坏分区开始重试，
约 111 秒完成下载，日志出现 `Header written`、`Successfully load srmodels`
和主题刷新完成。
